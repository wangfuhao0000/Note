...
然后提供了一个DriverManager来管理这些Driver的具体实现，就是使用了一个**CopyOnWriteArrayList**来存储多有的Driver，然后提供方法可进行Driver的注册。
...
核心就是这句Class.forName()触发了mysql驱动的加载，**当然这个加载就会使用双亲委派模型，选择合适的加载器进行加载**，我们看下mysql对Driver接口的实现：
...
public class Driver extends NonRegisteringDriver implements java.sql.Driver {
    ...
    static {
        try {
            DriverManager.registerDriver(new Driver()); //将自己注册到DirverManager中
...
那么，这个问题如何解决呢？按照目前情况来分析，这个mysql的drvier只有应用类加载器能加载，那么我们只要在启动类加载器中有方法获取应用程序类加载器，然后通过它去加载就可以了。这就是所谓的**线程上下文加载器**。
